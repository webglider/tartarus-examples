...
pea_handler(guid, (IP, Port), main) :- 
    %% If goal has been reached execute PTA
    (current_predicate(is_goal/0) -> writeln('Reached Goal!'), agent_execute(pta, (IP, Port), pta_handler) ;
...
pea_handler(guid, (IP, Port), handle_neighbours([(N,Dir)|T])) :-
