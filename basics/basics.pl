...
%% To safely close or stop a platform, simpy use:
platform_close.
...
%% created, it simply replaces all occurences of `guid` in the template with 
%% it's own name. For example if the agents name is `bar`, all occurences of
...
%% occurences of `guid` with `bar`, the name of the agent.
...
%% For consistency, agent names must be unique thoughout the network.
...
%% So you will get `bar1_localhost_50001` or somethind similar.
...
%% On platform 50001, you need to recconsult `foo.pl` and recreate the agent, as
...
%% They can be easily added, rmeoved and modified from the agent.
...
%% occurences of `guid` by the agent's name.
...
%% This string can later be modifed through agent code.
